...
<sub>Generated from a single source of truth by [`tools/feature_tree.py`](tools/feature_tree.py) — run `python3 tools/feature_tree.py --write` to regenerate.</sub>
...
├── Skills
│   ├── dex2jar — workbench
│   ├── apk-triage — fast first-pass
│   └── sensitive-api-audit
├── Convert
│   ├── DEX <-> JAR
│   ├── smali assemble / disassemble
│   └── jasmin
├── Analyze  (new in this fork)
